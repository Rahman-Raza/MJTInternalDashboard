  ...
  await axios.post('https://mjtbe.tk/assignjoborder', {JobPostingID: this.props.jobID, AssignedRecruiterName: recruiterName })
      .then(function (response) {
        ...
        if(response["status"]  == 200){
          ...
          self.setState({success: true, successDialog: "successfully assigned job order"})
          }
        }
...
handleAssignRecruiter (event){
 ...
 handleClose = () => {
  ...
  handlePhoneToggle = (warning_message) => {

    this.state.warning === false ? this.setState({warning: true, warningDialog: warning_message }) : this.setState({warningDialog: warning_message});
        ...
        <Grid container spacing={0}>
          <Grid item sm={7}>
            <Paper classes={{ root: this.props.classes.paper }} elevation={0}>
              <LetterHead editJobDescription={this.editJobDescription} jobID={this.props.jobID} handleAssignRecruiter={this.handleAssignRecruiter.bind(this)} jobData={this.state.jobData} cookieRole={this.props.cookies.get('Role')} />
